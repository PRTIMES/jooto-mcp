...
タスクの日付入力は、日付だけの場合は `YYYY-MM-DD`、時刻を含む場合はUTCの `YYYY-MM-DDTHH:mm:ssZ` に正規化してJooto APIへ送信します。`YYYY/MM/DD` や `YYYY-MM-DDTHH:mm:ss+09:00` など変換可能な入力も受け付けます。時刻付きの入力にはタイムゾーンが必要です。実在しない日付、タイムゾーンのない日時、その他の変換できない入力はエラーになります。
