...
チェックリストアイテムも開始日時と締切日時の両方を設定する必要があります。`jooto-create-checklist-item` で片方だけを指定した場合は未指定側にも同じ値を設定します。`jooto-update-checklist-item` で片方だけを指定した場合は、現在のアイテムを取得して既存の反対側の値を維持します。反対側が未設定の場合のみ、指定された値を両方に設定します。
