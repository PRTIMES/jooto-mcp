...
タスクの開始日時と締切日時は両方設定する必要があります。`jooto-create-task` で `start_date_time` と `deadline_date_time` の片方だけを指定した場合、MCPサーバーは未指定側にも同じ値を自動設定します。`jooto-update-task` で片方だけを指定した場合は、現在のタスクの反対側が設定済みなら変更対象だけを送信して既存値を維持します。ただし、新しい開始日時が締切日時より後、または新しい締切日時が開始日時より前になる場合は、両方を新しい値に揃えます。反対側が未設定の場合も、指定された値を両方に設定します。

チェックリストアイテムも開始日時と締切日時の両方を設定する必要があります。`jooto-create-checklist-item` で片方だけを指定した場合は未指定側にも同じ値を設定します。`jooto-update-checklist-item` で片方だけを指定した場合は、現在のアイテムの反対側が設定済みなら変更対象だけを送信して既存値を維持します。ただし、新しい開始日時が締切日時より後、または新しい締切日時が開始日時より前になる場合は、両方を新しい値に揃えます。反対側が未設定の場合も、指定された値を両方に設定します。
