...
タスクの開始日時と締切日時は両方設定する必要があります。`jooto-create-task` で `start_date_time` と `deadline_date_time` の片方だけを指定した場合、日付のみなら未指定側にも同じ日を設定し、時刻付きなら開始日時と締切日時が1分差になるよう補完します。`jooto-update-task` で片方だけを指定した場合は、現在のタスクの反対側が設定済みなら変更対象だけを送信して既存値を維持します。ただし、新しい開始日時が締切日時より後、または新しい締切日時が開始日時より前になる場合は、日付のみなら両方を新しい日に揃え、時刻付きなら新しい値を基準に1分差へ補正します。反対側が未設定の場合も同様に補完します。

チェックリストアイテムも開始日時と締切日時の両方を設定する必要があります。`jooto-create-checklist-item` で片方だけを指定した場合は、日付のみなら未指定側にも同じ日を設定し、時刻付きなら開始日時と締切日時が1分差になるよう補完します。`jooto-update-checklist-item` で片方だけを指定した場合は、現在のアイテムの反対側が設定済みなら変更対象だけを送信して既存値を維持します。ただし、新しい開始日時が締切日時より後、または新しい締切日時が開始日時より前になる場合は、日付のみなら両方を新しい日に揃え、時刻付きなら新しい値を基準に1分差へ補正します。反対側が未設定の場合も同様に補完します。

開始日時と締切日時を両方明示した場合でも、時刻まで完全に同一なら締切日時を開始日時の1分後へ補正します。日付のみの場合は同じ日を許容します。

タスクとチェックリストアイテムの日付入力は、日付だけの場合は `YYYY-MM-DD`、時刻を含む場合はJooto APIが要求するUTCの `YYYY-MM-DDTHH:mm:ss.0Z` に正規化して送信します。`YYYY/MM/DD` や `YYYY-MM-DDTHH:mm:ss+09:00` など変換可能な入力も受け付けます。時刻付きの入力にはタイムゾーンが必要です。実在しない日付、タイムゾーンのない日時、その他の変換できない入力はエラーになります。

タスクまたはチェックリストアイテムの日付を更新時にクリアする場合は、対象の `start_date_time` または `deadline_date_time` に空文字を明示的に指定します。日付を変更しない場合は、そのパラメータ自体を省略してください。空文字でのクリア時は現在値の取得や反対側の補完を行わず、指定されたパラメータだけを送信します。
